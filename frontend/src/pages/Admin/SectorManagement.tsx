...
export default function SectorManagement() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['sectors', page, searchName],
    queryFn: async () => {
      const params = new URLSearchParams({ page: String(page), pageSize: String(pageSize) })
      if (searchName) params.set('name', searchName)
      const res = await apiClient.get(`/sectors?${params}`)
      return res.data as PagedResult<Sector>
    },
  })

  const createMutation = useMutation({
    mutationFn: async (payload: { name: string; description?: string; isActive: boolean }) => {
      await apiClient.post('/sectors', payload)
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, payload }: { id: string; payload: { name: string; description?: string; isActive: boolean } }) => {
      await apiClient.put(`/sectors/${id}`, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['sectors'] })
      closeModal()
  ...
  if (isLoading) return <div className="p-8 text-center">{t('admin.sectorManagement.loading')}</div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center">
          ...
          {t('admin.sectorManagement.title')}
        </h1>
        <button
          onClick={openCreate}
            ...
            {data?.items.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                  ...
                  <button
                    onClick={() => {
                      if (confirm(t('common.deleteConfirmSector'))) {
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h3 className="text-lg font-medium text-gray-900 mb-4">
              {isCreating ? t('admin.sectorManagement.createSector') : t('admin.sectorManagement.editSector')}
            </h3>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="sector-is-active" className="text-sm font-medium text-gray-700">{t('common.active')}</label>
              </div>
            </div>
